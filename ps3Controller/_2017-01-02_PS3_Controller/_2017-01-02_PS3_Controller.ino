 ...
 *  
 *  -SAFETY IZE NO CONTROLLER ROUTINE (GATE LOW, 1V/OCT MIDDLE 'C')
 *  
...
PS3BT PS3(&Btd, 0x00, 0x15, 0x83, 0xE4, 0x34, 0x5A);        // This will also store the bluetooth address - this can be obtained from the dongle when running the sketch
//PS3USB PS3usb(&Usb);                                      // This will just create the instance
PS3USB PS3usb(&Usb,0x00,0x15,0x83,0xE4,0x34,0x5A);          // This will also store the bluetooth address - this can be obtained from the dongle when running the sketch
...
boolean           newNote             =     0;
boolean           kbdGate             =     0;                  //
boolean           lastKbdGate         =     0;
boolean           highPriority        =     0;                  //
unsigned int      kbdLEDval           =     0;
int               kbdLEDarray[16]     =     {};
int               kbdLEDarrayIndex    =     0;
int               brightness          =     0;
...
boolean           modeChangeAllowed   =     1;
boolean           modeTimerStart      =     0;
unsigned long     modeChangeTime      =     0;
boolean           kbdMode             =     1;
unsigned long     LEDfadeTime         =     0;
...
//-------------- BLUE TOOTH CONTROLLER ROUTINE --------------

  } else if (PS3.PS3Connected == 1) { 
    ...
    batteryState();                                     // call function to display controller battery level with built in LEDs 


//*********************JOYSTICKS********************************************************//
  
    rightY = (PS3.getAnalogHat(RightHatY));              // read the joystick 
    rightY = 255 - rightY;                               // and write the value via PWM
    analogWrite(RIGHT_Y_OUT, rightY);                    
                                                            
    rightX = (PS3.getAnalogHat(RightHatX));              // Y values are "upside down",
...
//********************* ACCELEROMETER X AND Y ******************************************//
    ...
    accelX = (PS3.getSensor(aX));                        // both X and Y are "upside down" and centered oddly
    ...
    accelY = (PS3.getSensor(aY));                        // want to change scaling
    accelY -= 380; 
...
//****************************** CHANGE MODES ******************************************************************************************************************
    ...
    if (modeChangeAllowed == true) {                                                // if you are allowed (allowed means you are not coming straight from switching modes)
      if (PS3.getButtonPress(START)) {                                              // you can switch mode between sequencer and keyboard mode by pressing and holding START for 5 seconds
        if (modeTimerStart == true) {                                               // if you just started pressing START
          modeChangeTime = millis();                                                // begin the timer
          modeTimerStart = false;
        }
          
        if (millis() - modeChangeTime >= 5000) {                                    // if you've been pressing it for 5 seconds    
          kbdMode = !kbdMode;                                                       // switch between keyboard and sequencer modes
          modeChangeAllowed = false;                                                // this forbids changing modes right away, so we don't oscillate back and forth between modes
        }
      } 
    } 
    
    if (!PS3.getButtonPress(START)) {                                               // it you are not pressing START
      modeTimerStart = true;                                                        // the next time you press it the timer will start over
      modeChangeAllowed = true;                                                     // and you are allowed to enter or exit programming mode   
    }

//************************** MUSICAL KEYBOARD ROUTINE ******************************************************

    if (kbdMode == 1) {                             // if kbdMode is true, do keyboard stuff
      ...
      if (highPriority == 1) {
        ...
        if (PS3.getButtonPress(SQUARE)) {        // C
          kbdNote = 1;
          kbdGate = 1;
        }
        
        if (PS3.getButtonPress(TRIANGLE)) {      // D
          kbdNote = 3;
          kbdGate = 1;
        }
        
        if (PS3.getButtonPress(CROSS)) {         // E
          kbdNote = 5;
          kbdGate = 1;
        }
      
        if (PS3.getButtonPress(CIRCLE)) {        // F
          kbdNote = 6;
          kbdGate = 1;
        }
      
        if (PS3.getButtonPress(L1)) {            // G
          kbdNote = 8;
          kbdGate = 1;
        }
      
        if (PS3.getButtonPress(R1)) {            // A
          kbdNote = 10;
          kbdGate = 1;
        }
      
        if (PS3.getButtonPress(L2)) {            // B
          kbdNote = 12;
          kbdGate = 1;
        }
      
        if (PS3.getButtonPress(R2)) {            // C + 1 octave
          kbdNote = 13;
          kbdGate = 1;
        }

      } else {                                   // below is low note priority
      ...
      if (kbdGate == 1) {
        ...
        if (PS3.getButtonPress(LEFT)) {          // 1/2 step flat
          kbdNote -= 1;
        }
      
        if (PS3.getButtonPress(RIGHT)) {         // 1/2 step sharp
          kbdNote += 1;
        }
      
        if (PS3.getButtonPress(UP)) {            // up one octave
          kbdNote += 12;
        }
      
        if (PS3.getButtonPress(DOWN)) {          // down one octave
          kbdNote -= 12;
        }
      }

      
      setOutput(kbdNote + 36);                      // play the note, push it up three octaves so it is in the middle of its range
     
      kbdLEDval = 255 - ((kbdNote + 11) * 6);       // scale the kbd note for LED displaying



      if ((kbdNote != lastKbdNote) || 
          kbdGate != lastKbdGate) {                 // make a note (heh) of when you've played a new note
        newNote = 1;
      } else {
        newNote = 0;
      }
      ...
      if (kbdGate == 1 && newNote == 1) {           // if you just played a new note

        brightness = 0;                             // set the brightness to zero and the color to the scaled note value
        
        for (int i = 0; i < 16; i ++) {
          leds[i] = CHSV(kbdLEDval + (i * 3), 255, brightness);
        }

        FastLED.show();
      } 
      
      else if (kbdGate == 1 && newNote == 0
               && kbdLEDarrayIndex < 16 ) {         // if you are still holding the note and have not reached the last LED yet  

        if (brightness <= 100) {

          brightness ++;                            // keep the color the same but ramp up the brightness 
    ...
    } else {                                      // if kbdMode is false, do the sequencer routine below
...
//*********************** PATTERN GENERATOR AND SEQUENCER ***********************************************//
      ...
      if (PS3.getButtonClick(RIGHT)) {                          // move the editable slot position right
      ...
      if (PS3.getButtonClick(LEFT)) {                           // or left
        if (slotOutatime == true) {
          slot ++;
        }
        slot --;
      ...
      if (slotOn == true) {                                     // if you have not timed out
        if (PS3.getButtonClick(SQUARE)) {                       // you can edit the sequence
        ...
        if (PS3.getButtonClick(SELECT)) {                       // and the gate pattern
  ...
  //*********************** TAP TEMPO CLOCK **************************************************//
      ...
      if (PS3.getButtonClick(START)) {                              // here we set how many times the
      ...
      if (PS3.getButtonClick(CIRCLE)) {                             // tap!                         
      ...
      if (PS3.getButtonPress(TRIANGLE) && clockPeriod > 1) {          // speed up the clock generator
        if (PS3.getAnalogButton(TRIANGLE) > 200) {                    // if you push hard, speed up more
      ...
      if (PS3.getButtonPress(CROSS) && clockPeriod < 20000) {         // slow down the clock generator
        if (PS3.getAnalogButton(CROSS) > 200) {                       // if you push hard, slow down more
          clockAdjust += (adjustment * 10);
        } else {
          clockAdjust += adjustment;    
        }
      ...
      if (slotOn == false) {                                             // if you are not editing a step, you can
        if (PS3.getButtonClick(SQUARE)) {                                // reset the sequencer by pressing SQUARE
            reset = true;
        }
      }
      ...
      if (clockOn) {              
        if (toggle) {
          if (risingEdge) {                                              // at the rising edge of a clock cycle
            ...
            setOutput(oneVoltPerOct);                                    // send the 1v/oct signal to the DAC
  ...
  //************************ D-PAD OCTAVE SWITCH *************************************************//
      ...
      if (slotOn == false) {                       // octave is used while the sequence is playing, only works when slot is inactive
        if(PS3.getButtonClick(UP)) {               // increment the octave
          octave += 12;
        }
        
        if(PS3.getButtonClick(DOWN)) {             // decrement the octave
          octave -= 12;
        }
      }
      ...
      if (PS3.getButtonPress(UP)) {               // octaveNow is used while editing the sequence, only works when slot is active
        octaveNow = 48;
  ...
  //*********************SHOULDER BUTTONS************************************************//
      ...
      if (PS3.getButtonPress(L1) || PS3.getButtonPress(R1) ||           // if any shoulder buttons are pressed
        PS3.getButtonPress(L2) || PS3.getButtonPress(R2)) {
          shoulderPressed = true;
          digitalWrite(SHOULDER_GATE, HIGH);                            // send the shoulder gate high  
          shoulderNow = 0;  
          
          if(PS3.getButtonPress(L1)) {                                  // and update the new value by adding each bit to shoulder value
...
//****************************** CHANGE MODES ******************************************************************************************************************
    ...
    if (modeChangeAllowed == true) {                                                // if you are allowed (allowed means you are not coming straight from switching modes)
      if (PS3usb.getButtonPress(START)) {                                           // you can switch mode between sequencer and keyboard mode by pressing and holding START for 5 seconds
        if (modeTimerStart == true) {                                               // if you just started pressing START
          modeChangeTime = millis();                                                // begin the timer
          modeTimerStart = false;
        }
...
//************************** MUSICAL KEYBOARD ROUTINE ******************************************************
    ...
    if (kbdMode == 1) {                             // if kbdMode is true, do keyboard stuff
      ...
      if (highPriority == 1) {
        ...
        if (PS3usb.getButtonPress(SQUARE)) {        // C
          kbdNote = 1;
          kbdGate = 1;
        }
        
        if (PS3usb.getButtonPress(TRIANGLE)) {      // D
          kbdNote = 3;
          kbdGate = 1;
        }
        
        if (PS3usb.getButtonPress(CROSS)) {         // E
          kbdNote = 5;
          kbdGate = 1;
        }
      
        if (PS3usb.getButtonPress(CIRCLE)) {        // F
          kbdNote = 6;
          kbdGate = 1;
        }
      
        if (PS3usb.getButtonPress(L1)) {            // G
          kbdNote = 8;
          kbdGate = 1;
        }
      
        if (PS3usb.getButtonPress(R1)) {            // A
          kbdNote = 10;
          kbdGate = 1;
        }
      
        if (PS3usb.getButtonPress(L2)) {            // B
          kbdNote = 12;
          kbdGate = 1;
        }
      
        if (PS3usb.getButtonPress(R2)) {            // C + 1 octave
          kbdNote = 13;
          kbdGate = 1;
        }
      ...
      if (kbdGate == 1) {
        ...
        if (PS3usb.getButtonPress(LEFT)) {          // 1/2 step flat
          kbdNote -= 1;
        }
      
        if (PS3usb.getButtonPress(RIGHT)) {         // 1/2 step sharp
          kbdNote += 1;
        }
      
        if (PS3usb.getButtonPress(UP)) {            // up one octave
          kbdNote += 12;
        }
      
        if (PS3usb.getButtonPress(DOWN)) {          // down one octave
          kbdNote -= 12;
        }
      }
...
void setOutput(unsigned int val) {                      // this writes the 1v/oct signal to the external DAC

  val = (4096 - (val * 45));                            // this scales and inverts the output, DAC feeds an inverting opamp
                                                        // which flips it right side up again
