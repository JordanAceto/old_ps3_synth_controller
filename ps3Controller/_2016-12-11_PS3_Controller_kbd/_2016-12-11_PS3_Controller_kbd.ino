...
//************************** MUSICAL KEYBOARD ROUTINE ******************************************************
    ...
    if (kbdMode == 1) {                             // if kbdMode is true, do keyboard stuff
      ...
      kbdNote ++;                                   // add one to make the kbd note the same root as the sequencer, this dumb, I just don't want to deal with figuring out why they are different right now
    
      setOutput(kbdNote + 36);

      Serial.println(kbdNote + 36);
      ...
      if (clockOn) {              
        if (toggle) {
          if (risingEdge) {                                              // at the rising edge of a clock cycle
            ...
            Serial.println(oneVoltPerOct);                                                             // an inverting opamp scales it further
            if (pattern[patIndex]) {      
              stepOn = true;                                    
            }
