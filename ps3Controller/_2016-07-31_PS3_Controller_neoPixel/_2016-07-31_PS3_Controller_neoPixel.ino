 ...
 *  
 *  -DAC MULTIPLIER OVERFLOWS AT HIGH AND LOW END
 *  
...
int               seqLEDscaled[16]    =     {};               //array of values for LEDs, colors represent 1V/oct value
...
unsigned int      slot                =     0;                //position of pattern generator for editing, 0 to 15. LED at this steo will blink while editable
...
int               sequence[16]        =     {};               //edited by selecting position with LEFT/RIGHT, programming note with shoulder buttons and UP/DOWN, and writting with SQUARE 
...
int               accelX              =     127;                //value of accelerometer X and Y values, 0 - 255
int               accelY              =     127;
...
for (int i = 0; i < 16; i ++) {          //make a beautiful rainbow at startup
...
pinMode(A1, OUTPUT);                    //serial data to LEDs
...
//------------(includes bracket at end of main loop)------
  ...
  if (slotOn == true) {                                          //if you have not timed out
      if (PS3usb.getButtonClick(SQUARE)) {                       //you can edit the gate pattern
      ...
      if (PS3usb.getButtonClick(SELECT)) {                       //and the sequence
  ...
  FastLED.show();                                                               //here is where we actually set all the LEDs
...
*/
  ...
  if (clockOn) {              
      if (toggle) {
          if (risingEdge) {
              ...
              setOutput(4096 - (oneVoltPerOct * 45));
  ...
  accelX = (PS3usb.getSensor(aX));                     //both X and Y are "upside down" 
  accelX -= 380;                                       //
  accelX = constrain(accelX, 0, 255);
...
//************************ D-PAD OCTAVE SWITCH *************************************************//
  ...
  if (slotOn == false) {                       //octave is used while the sequence is playing
  ...
  if (PS3usb.getButtonPress(UP)) {             //octaveNow is used while editing the sequence
      octaveNow = 48;
