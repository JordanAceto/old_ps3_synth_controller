...
//        NEOPIXEL LED USING FASTLED LIBRARY

unsigned long     slotLedTimeout      =     0;
boolean           slotLedBlink        =     false;
boolean           ledPattern[16]      =     {};
int               seqLEDscaled[16]    =     {};
int               seqInit             =     0;

//        SEQUENCER AND PATTERN GENERATOR 

unsigned int      rawPatLength        =     14;               //because modulo
...
boolean           slotOn              =     false;            //while true the slot selector is active, and you can turn individual steps on and off
...
//------------(includes bracket at end of main loop)------
  ...
  for (int i = 0; i < patLength; i++) {
      ...
      if (slotOn) {
          ledPattern[slot] = false;
      }
      
      ledPattern[patIndex] = false;
...
///*
  if (slotOn) {
      if ((millis() - slotLedTimeout) > 200) {
          slotLedBlink = !slotLedBlink;
          slotLedTimeout = millis();
      }
      if (slotLedBlink) {
          leds[slot] = CHSV(seqLEDscaled[slot], 255, 75); 
          FastLED.show();
      } else {
...
*/
  ...
  for (int i = 0; i < patLength; i++) {
      if (ledPattern[i]) {
          leds[i] = CHSV(seqLEDscaled[i], 255, 75);
          FastLED.show();
      } else if (i != slot) {
          leds[i] = CHSV(seqLEDscaled[i], 255, 25);
          FastLED.show();
      }
  }
  ...
  if (clockOn) {              
      if (toggle) {
          if (risingEdge) {
              ...
              if (pattern[patIndex]) {
                  stepOn = true;
                  //leds[patIndex] = CHSV(sequence[patIndex], 187, 175);
                  //FastLED.show();
              //} else {
                  //leds[patIndex] = CRGB::Black;
                  //FastLED.show(); 
              }
